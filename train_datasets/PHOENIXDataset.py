...
class PhoenixDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # translation #
        trg_trans = self.df.iloc[idx]['translation']
        trg_gloss = self.df.iloc[idx]['orth']

        return image_names, ipt_len, trg_labels, trg_length, trg_trans, trg_gloss
...
def collator(data, data_augmentation):
  ...
  return batch, torch.tensor(vid_lens, dtype=torch.int32), targets, torch.tensor(trg_lens, dtype=torch.int32), trg_trans, trg_gloss, max_ipt_len
